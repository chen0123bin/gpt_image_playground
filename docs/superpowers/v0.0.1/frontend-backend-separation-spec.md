...
本版本将项目演进为同仓库前后端分离架构：前端继续负责界面、配置、历史记录和任务展示；新增 Node.js 后端作为 API 能力层，前端调用自有后端，后端再调用真实图片服务商。
...
- 不重写全部 UI。
- 不一次性实现完整任务队列。
- 不删除现有 `/api-proxy/` 能力；第一阶段可以保留为兼容或回退路径。
...
第一阶段后端保持轻量，不建立独立插件系统。与现有前端类型重复的地方优先提取最小共享类型，避免为了抽象而大规模搬迁代码。

## API 边界

前端新增一个“服务端 API 模式”。开启后，图片生成请求发送到同源后端：
...
5. 后端组装真实服务商请求，并携带 `Authorization: Bearer <apiKey>`。
...
现有 `/api-proxy/` 可以保留，用于兼容 Docker 或本地开发中的旧配置。新实现的主路径应是 `/api/openai-compatible/*`，前端设置中可以新增或复用“API 代理/服务端转发”开关。
...
- Node 后端提供静态文件托管能力，直接服务 Vite 构建产物。
- Docker 镜像启动 Node 服务，而不是仅启动 Nginx 静态服务。
- 如果仍保留 Nginx，可让 Nginx 反向代理到 Node 后端，但不再依赖 Nginx 实现 API 转发核心逻辑。

## 测试策略

- 为后端 URL 构建、请求校验、错误归一化、图片 URL 转 data URL 添加单元测试。
- 为前端 API 调用分支添加测试，确认开启服务端模式时请求 `/api/openai-compatible/*`。
...
- 浏览器网络面板中图片生成请求指向同源后端 `/api/openai-compatible/*`。
